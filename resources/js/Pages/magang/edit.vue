<template>
    <Head title="Edit Magang" />
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Magang</h2>
        </template>
                    ...
                    <div class="p-6 bg-white border-b border-gray-200">
                        <form @submit.prevent="submit">
                            <div class="grid grid-cols-1 gap-4">
                                ...
                                <div>
                                    <label for="sertifikat_link" class="block text-sm font-medium text-gray-700">Sertifikat Link</label>
                                    <input
                                        id="sertifikat_link"
                                        v-model="form.sertifikat_link"
                                        type="url"
                                        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                    />
                                </div>
...
const form = useForm({
    perusahaan: props.magang.perusahaan,
    durasi: props.magang.durasi,
    divis: props.magang.divis,
    tugas: props.magang.tugas,
    sertifikat_link: props.magang.sertifikat_link
});

const submit = () => {
    form.put(route('magang.update', props.magang.id), {
        onSuccess: () => {
            form.reset();
        },
        onError: (errors) => {
